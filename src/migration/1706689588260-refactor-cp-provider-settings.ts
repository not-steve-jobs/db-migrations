...
async function setConvertedCurrencies(queryRunner: QueryRunner): Promise<void> {
  ...
  for (const { providerId, convertedCurrency } of convertedCurrencies) {
    await queryRunner.query(
      'UPDATE cp_providers SET convertedCurrency = ? WHERE id = ?',
      [convertedCurrency, providerId]
    );
  }
}
...
async function setIsPaymentAccountRequired(queryRunner: QueryRunner): Promise<void> {
  ...
  if (isPaymentAccountRequiredRows?.length) {
    ...
    await queryRunner.query(
      'UPDATE cp_providerMethods SET isPaymentAccountRequired = true WHERE id IN (?)',
      [pmIds]
    );
  }
...
async function setIsPaymentAccountRequiredByProvider(queryRunner: QueryRunner, value: boolean, code: string): Promise<void> {
  await queryRunner.query(

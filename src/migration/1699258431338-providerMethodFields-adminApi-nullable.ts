...
export class ProviderMethodFieldsAdminApiNullable1699258431338 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(`CREATE UNIQUE INDEX \`CPPMFUniqueProviderMethodIdTypeKey\` ON \`cp_providerMethodFields\` (\`providerMethodId\`, \`transactionType\` ,\`key\`)`);
    await tolerateQuery(queryRunner, `DROP INDEX \`CPPMFUniqueProviderMethodIdAdminApiIdKey\` ON \`cp_providerMethodFields\``);

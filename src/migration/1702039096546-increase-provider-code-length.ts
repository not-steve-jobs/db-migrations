...
async function changeCpProvidersCodeLength(queryRunner: QueryRunner, length: number): Promise<void> {
  ...
  await queryRunner.query(`ALTER TABLE cp_credentials ADD CONSTRAINT fkCPCProviderCodeIdx FOREIGN KEY (providerCode) REFERENCES cp_providers(code) ON DELETE RESTRICT ON UPDATE CASCADE;`);
}

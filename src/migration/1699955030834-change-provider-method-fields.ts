...
export class ChangeProviderMethodFields1699955030834 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await tolerateQuery(queryRunner, `DROP INDEX \`CPPMFUniqueProviderMethodIdTypeKey\` ON \`cp_providerMethodFields\``);
    await queryRunner.query(`ALTER TABLE \`cp_providerMethodFields\` CHANGE \`providerMethodId\` \`entityId\` varchar(36) NOT NULL`);
    await queryRunner.query(`ALTER TABLE \`cp_providerMethodFields\` ADD \`entityType\` varchar(36) NOT NULL DEFAULT 'providerMethod'`);
    ...
    await queryRunner.query(`ALTER TABLE \`cp_fieldOptions\` CHANGE \`providerMethodFieldId\` \`fieldId\` varchar(36) NOT NULL`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`cp_providerMethodFields\` CHANGE \`entityId\` \`providerMethodId\` varchar(36) NOT NULL`);
    ...
    // set cp_fieldOptions relation with cp_Fields
    await queryRunner.query(`ALTER TABLE \`cp_fieldOptions\` CHANGE \`fieldId\` \`providerMethodFieldId\` varchar(36) NOT NULL`);
    await queryRunner.query(`CREATE UNIQUE INDEX \`CPFOUniqueFieldKey\` ON \`cp_fieldOptions\` (\`providerMethodFieldId\`, \`key\`)`);
    await queryRunner.query(`ALTER TABLE \`cp_fieldOptions\`

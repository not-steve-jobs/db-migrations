...
export class CpExpandFieldsUniqueIndex1695814355509 implements MigrationInterface {
  name = 'CpExpandFieldsUniqueIndex1695814355509';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE UNIQUE INDEX \`CPPMFUniqueProviderMethodIdAdminApiIdKey\` ON \`cp_providerMethodFields\` (\`providerMethodId\`, \`key\`, \`adminApiId\`)`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE UNIQUE INDEX \`CPPMFUniqueProviderMethodIdAdminApiId\` ON \`cp_providerMethodFields\` (\`providerMethodId\`, \`adminApiId\`)`);
    await queryRunner.query(`DROP INDEX \`CPFOUniqueFieldKey\` ON \`cp_fieldOptions\``);
    await queryRunner.query(`CREATE UNIQUE INDEX \`CPFOUniqueFieldKeyValue\` ON \`cp_fieldOptions\` (\`providerMethodFieldId\`, \`key\`, \`value\`)`);
    await queryRunner.query('SET FOREIGN_KEY_CHECKS = 1');
    await queryRunner.query(`ALTER TABLE \`cp_providerMethodFields\` DROP COLUMN \`pattern\``);
